...
分别是正确的密码和系统允许尝试的次数。随后每行给出一个以回车结束的非空字符串，是用户尝试输入的密码。
...
Welcome in
"""
#注意逻辑 一旦达到最大尝试次数就立即输出Account locked同时跳出循环 
...
def main():
    ...
    user_password = input_string[0]
    user_attempts = int(input_string[1])

    count = 0
    password_list = getPasswords()
    for i in password_list:
        if(i != user_password and count < user_attempts):
            print("Wrong password: %s" %i)
            count += 1
            if(count == user_attempts):
                print("Account locked")
                break
        elif(i == user_password and count < user_attempts):
            print("Welcome in")
            break
